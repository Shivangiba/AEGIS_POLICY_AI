...
### `backend/.env`
Stores runtime secrets and configuration:
```
GROQ_API_KEY="gsk_..."              # Groq API key for LLM access
EMBEDDING_MODEL="sentence-transformers/all-MiniLM-L6-v2"
CHUNK_SIZE=800
CHUNK_OVERLAP=150                   # Note: config.py defaults to 200, this overrides to 150
...
| Component | Purpose |
|---|---|
| `Sidebar.tsx` | Collapsible session sidebar; filename/chunk count; question history; "New Chat" button; mobile overlay |
...
- **Sessions are ephemeral** — if the FastAPI server restarts, all sessions are lost and users must re-upload their PDF.
- **The Groq API key is in `.env`** — treat it as sensitive; never commit `.env` to version control.
- **No database** — no persistence layer exists; everything is in-memory for privacy.
- **No authentication** — any client knowing the `session_id` UUID can query that session.
